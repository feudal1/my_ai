...
namespace tools
{
    partial class Program
    {
        ...
        static void MergeDwgCommand(string[] args)
        {
            if (args.Length > 1)
            {
                merge_dwg.run(args[1]);
            }
            else
            {
                Console.WriteLine("请提供 DWG 文件路径参数");
            }
        }

        [Command("folder2merge", Description = "批量合并文件夹内 DWG 文件", Parameters ="无", Group = "cad")]
        static void Folder2MergeCommand(string[] args)
        {
            ...
            if (files != null)
            {
                ...
                foreach (var file in files)
                {
                    merge_dwg.run(file, 0.0, fileIndex);
                    fileIndex++;
                }
            }
        }
