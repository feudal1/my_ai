...
def detect_like_favorite() -> ToolResponse:
    """
    检测点赞和收藏按钮
    自动截取当前屏幕进行检测
    """
    ...
    if not model_path.exists():
        return ToolResponse(f"错误: 模型文件不存在 - {model_path}")
    ...
    try:
        ...
        if not detections:
            return ToolResponse("未检测到点赞或收藏按钮")
        ...
        return ToolResponse(result_str.strip())
        
    except Exception as e:
        return ToolResponse(f"检测过程中出错: {str(e)}")
