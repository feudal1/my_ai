...
def draw_dimensions(acad, box, color_index=3, hide_border=False):
    """
    在包围框上绘制长度和宽度的尺寸标注
    :param acad: Autocad实例
    :param box: 包围框信息字典
    ...
    try:
        ...
        print(f"已绘制尺寸标注 (蓝色): 宽度={box['width']:.1f}, 高度={box['height']:.1f}")
...
def draw_green_dimensions_only(acad, box):
    """
    仅绘制绿色尺寸标注，不绘制边界框
    :param acad: Autocad实例
    :param box: 包围框信息字典
    ...
    try:
        ...
        print(f"已绘制绿色尺寸标注: 宽度={box['width']:.1f}, 高度={box['height']:.1f}")
        
        return [dim_x, dim_y]
    except Exception as e:
        print(f"绘制绿色尺寸标注时出错: {e}")
        return None

def analyze_rotate_and_draw(acad, entities, rotate_to_align=True, draw_boxes=False, green_dimensions_only=True):
    """分析、旋转实体并绘制包围框
    ...
    :param green_dimensions_only: 是否仅绘制绿色尺寸标注
    """
    ...
    if aabb_original:
        print("\n原始轴对齐包围盒 (AABB):")
        print(f"  尺寸: {aabb_original['width']:.1f} x {aabb_original['height']:.1f}")
        print(f"  面积: {aabb_original['area']:.1f}")
    ...
    if obb:
        print("\n最小包围盒 (OBB):")
        print(f"  尺寸: {obb['width']:.1f} x {obb['height']:.1f}")
        print(f"  面积: {obb['area']:.1f}")
        print(f"  旋转角度: {obb['angle']:.1f}度")
        print(f"  中心点: ({obb['center'][0]:.1f}, {obb['center'][1]:.1f})")
        ...
        if rotate_to_align and len(entities) > 0:
            ...
            if aabb_rotated:
                print("\n旋转后轴对齐包围盒:")
                print(f"  尺寸: {aabb_rotated['width']:.1f} x {aabb_rotated['height']:.1f}")
                print(f"  面积: {aabb_rotated['area']:.1f}")
                
                # 仅绘制旋转后的AABB绿色尺寸标注
                if green_dimensions_only and not draw_boxes:
                    draw_green_dimensions_only(acad, aabb_rotated)
                    print("  已绘制旋转后AABB绿色尺寸标注")
                ...
                print(f"\n验证结果:")
                print(f"  宽度差异: {width_diff:.1f}")
                print(f"  高度差异: {height_diff:.1f}")
                print(f"  面积差异: {area_diff:.1f}")
...
def obb_and_rotate_cmd():
    ...
    try:
        ...
        GREEN_DIMENSIONS_ONLY = True   # 是否仅绘制绿色尺寸标注
        
        result = analyze_rotate_and_draw(acad, entities, ROTATE_TO_ALIGN, DRAW_BOUNDING_BOXES, GREEN_DIMENSIONS_ONLY)
        
        if result:
            if GREEN_DIMENSIONS_ONLY and not DRAW_BOUNDING_BOXES:
                print("\n分析完成! 仅绘制绿色尺寸标注，未绘制其他任何内容")
